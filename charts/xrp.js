...
socket1.on(chartTime, (pl) => {
 ...
 socket.on('KLINE_XRP_CHANGE', (pl) => {
  ...
  document.getElementById('livePrice').innerHTML = "XRP|USD " + Math.round(pl.price * 100) / 100;
...
function chart4h() {
  ...
  chartTime = "KLINE_XRP_4h";

  fetch(`http://127.0.0.1:9665/fetchAPI?endpoint=https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval=4h&limit=50000`)
    .then(res => res.json())
    .then(data => {
      const cdata = data.map(d => {
        return { time: d[0] / 1000, open: parseFloat(d[1]), high: parseFloat(d[2]), low: parseFloat(d[3]), close: parseFloat(d[4]) }
...
function chart1m() {
  ...
  fetch(`http://127.0.0.1:9665/fetchAPI?endpoint=https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval=1m&limit=50000`)
    .then(res => res.json())
    .then(data => {
      const cdata = data.map(d => {
        return { time: d[0] / 1000, open: parseFloat(d[1]), high: parseFloat(d[2]), low: parseFloat(d[3]), close: parseFloat(d[4]) }
...
function marketOrderB() {
  ...
  if (total < buyOrder) {
    ...
    socket.once('KLINE_XRP_1d', (pl) => {
      ...
      //log(order1amount);
      document.getElementById('order1amount').innerHTML = order1amount + " XRP";
